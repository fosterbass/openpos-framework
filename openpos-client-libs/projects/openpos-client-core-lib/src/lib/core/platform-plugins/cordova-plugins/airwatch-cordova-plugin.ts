...
export class AirwatchCordovaPlugin implements IPlatformPlugin, IEnterpriseConfig {
    configName = 'AirwatchCordovaPlugin';
            ...
            return this.cordovaService.onDeviceReady.pipe(
                ...
                tap(() => {
                    ...
                    this.jmcAirwatchPlugin.setSDKEventCallback((eventName, eventData) => {
                        this.handleSDKEventCallback(eventName, eventData, observer);
                    });
                })
            ).subscribe();
        });
    ...
    private handleSDKEventCallback(eventName, eventData, observer) {
        if (eventName === 'initSuccess') {
            ...
            let reason = '?';
            if (eventData) {
                reason = eventData.error ? eventData.error : JSON.stringify(eventData);
            }
            observer.error(`Airwatch SDK failed to initialize. Reason: ${reason}`);
        }
    }
}
