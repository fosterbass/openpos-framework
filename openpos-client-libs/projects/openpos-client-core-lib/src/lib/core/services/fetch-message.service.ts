...
export class FetchMessageService {
    constructor(
        ...
        private personalizer: PersonalizationService
    ) {
    ...
    private handleFetchMessage(message: FetchMessage) {
        const url = `${this.discovery.getApiServerBaseURL()}/app/${this.personalizer.getAppId$().getValue()}/node/${this.personalizer.getDeviceId$().getValue()}/message/${message.messageIdToFetch}`;
