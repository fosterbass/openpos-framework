...
public class EndpointFilterManager {
    ...
    public final static Version MAX_VERSION = new Version(99999,99999,99999);
    public final static Version MIN_VERSION = new Version(0,0,0);
    ...
    protected EndpointFilterTemplate buildFilterTemplate(Map.Entry<String, Object> beanEntry, Method method) {
        ...
        Version minVersion = !StringUtils.isEmpty(minVersionString) ?
                Version.parse(minVersionString) : MIN_VERSION;
        Version maxVersion = !StringUtils.isEmpty(maxVersionString) ?
    ...
    public Object filterRequest(RequestContext context) {
        ...
        Version version = !StringUtils.isEmpty(context.getVersion()) ?
                Version.parse(context.getVersion()) :
                VERSION_ZERO;

        for (EndpointFilterTemplate filterTemplate : requestFilterTemplates) {

            if (!StringUtils.isEmpty(filterTemplate.getPath())
            ...
            if (version.isGreaterThan(filterTemplate.getMinimumVersion())
                ...
                if (filterTemplate.getOutputType().isAssignableFrom(context.getInputType())) {
                    ...
                    try {
                        ...
                        Object result = invokeFilter(filterTemplate, context, argument);
                        return result;
                    } catch (Exception ex) {
    ...
    protected Object filterResponse(EndpointInvocationContext context, EndpointFilterTemplate filterTemplate, Object argument) {
        ...
        if (!StringUtils.isEmpty(filterTemplate.getPath())
                && !filterTemplate.getPath().equals(context.getEndpointPath())) {
            return null;
        }
