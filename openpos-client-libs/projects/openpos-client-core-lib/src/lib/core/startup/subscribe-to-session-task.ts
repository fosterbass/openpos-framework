...
import {Observable, throwError, Subject, concat, defer} from 'rxjs';
...
export class SubscribeToSessionTask implements IStartupTask {
    ...
    execute(data: StartupTaskData): Observable<string> {
        return Observable.create((messages: Subject<string>) => {
            messages.next('started');
            if (!this.session.connected()) {
                console.info(`[SubscribeToSessionTask] session not connected`)
                const subscribe = defer(async () => {
                    console.info(`[SubscribeToSessionTask] Running deferred async func`)
                ...
                return concat(
                    subscribe,
                    Observable.create((obs: Subject<string>) => {
