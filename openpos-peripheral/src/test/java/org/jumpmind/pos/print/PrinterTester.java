...
public class PrinterTester {
    ...
    private static EscpPOSPrinter createPrinter() {
...
//        settings.put("usbVendorId", 0x08a6); // TOSHIBA
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            EscpPOSPrinter printer = createPrinter();
...
//            byte[] printStoredLogo = {29, 40, 76, 6, 0, 48, 69, 48, 48, 24, 24};
           // byte[] printStoredLogo =  {29, 40,  76, 6, 0, 48, 69, 48, 48, 1, 1};

            byte[] printStoredLogo =  {29, 40,  76, 6, 0, 48, 69, 48, 48, 1, 1};
            printer.printNormal(0, printer.printerCommands.get(PrinterCommands.ALIGN_LEFT));

                    printer.getPeripheralConnection().getOut().write("Hello\n".getBytes());
            printer.printNormal(0, printer.printerCommands.get(PrinterCommands.ALIGN_CENTER));
            printer.getPeripheralConnection().getOut().write(printStoredLogo);
            printer.printNormal(0, printer.printerCommands.get(PrinterCommands.ALIGN_LEFT));
            printer.getPeripheralConnection().getOut().write("Goodbye\n\n\n\n\n\n\n".getBytes());
