...
import {IPrinter} from '../printers/printer.interface';
...
export class ZebraBluetoothPrinterCordovaPlugin implements IPlatformPlugin, IPrinter {
    printerMAC: string;
    printer: any;
    bluetoothSerial: any;
    ...
    getPrinterAddress(message: any) {
        if (!!message.additionalFields && !!message.additionalFields.bluetoothDeviceName) {
            ...
            if (!!this.bluetoothDevices) {
                const device = this.bluetoothDevices.filter(d => this.printerName.toUpperCase() === d.name.toUpperCase());
                if (device.length > 0) {
     ...
     * @param printerAddress  bluetooth address of the printer
     * @param message the data to send to the printer
     */
    ...
    print(data: string): Observable<void> {
        this.bluetoothPrint(this.getPrinterAddress(data), data);
        return EMPTY;
    }
}
