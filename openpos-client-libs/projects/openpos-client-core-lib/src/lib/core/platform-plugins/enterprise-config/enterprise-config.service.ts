            ...
            const filterConfigs =  new Observable<string>( messages => {
                ...
                this.enterpriseConfigs.forEach( cfg => {
                    if ( ! cfg.configPresent() ) {
                        ...
                        messages.next(`Removing ${cfg.name()}`);
                    } else {
                        messages.next(`Found ${cfg.name()}`);
                    }
                });
        ...
        return new Observable<string>(messages => {
            if (this.enterpriseConfigs) {
                ...
                const configs = this.enterpriseConfigs.map(eCfg => new Observable<string>(observer => {
                    eCfg.getConfiguration().subscribe( cfg => {
                        if (cfg) {
                            try {
                                this.combinedConfig = Object.assign(this.combinedConfig, JSON.parse(cfg));
                            } catch(e) {
                                observer.error(`Failed to parse config from ${eCfg.name()} into JSON. config: ${cfg}`);
                            }
                        }
                        if (this.enterpriseConfigs.length > 1) {
                            observer.next(`Added/overlaid configuration from ${eCfg.name()}`)
                        }
                        observer.complete();
                    });
                }));
