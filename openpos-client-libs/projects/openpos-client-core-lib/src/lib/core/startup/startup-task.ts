...
function getTaskName(task: StartupTaskLike): string {
  function hasName(inst: any): inst is { name: string } {
    return inst.name;
  }
...
class ConsoleLogAugmenter {
  ...
  private static _hookConsole(name: string, method: keyof Console & ('info' | 'debug' | 'error' | 'warn' | 'log')): () => void {
    ...
    console[method] = (...args: any[]) => {
      if (args.length > 0) {
        ...
        ConsoleLogAugmenter._rootConsole[method](...newArgList);
      }
    };
