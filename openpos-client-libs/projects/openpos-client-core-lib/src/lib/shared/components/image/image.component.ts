...
/**
 * A wrapper around an `<img>` element that will run the URLs through
 * the image service and swap in an alternate image if the primary results in an error.
 */
@Component({
  selector: 'app-image',
...
export class ImageComponent {
  ...
  imageError(image) {
    const imgSource = this.imageSources[0];
    let imgUrl;
    switch (imgSource) {
